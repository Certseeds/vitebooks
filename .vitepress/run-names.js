...
const level2 = async (line) => {
    const postObject = {
        "model": "qwen2.5:14b",
        "prompt": `hey, 请阅读下面用三个 at 符号括住的一段话, 这摘录自一篇战锤30k荷鲁斯叛乱系列小说中. 希望你在读完之后, 从中提取出人名, 并以json格式返回, 要求逐个放入names这个列表中, 只需要人名, 不需要'他','她','它','您'等第三人称代词, 不需要'兄弟', '姐妹'这样的亲属称谓, 不需要'大人', '指挥官', '队长', '大师', '总管'之类的职位称呼, 不需要A的B这样的连词(至少要将其拆分为A和B), 不需要车辆, 飞行器的名称(比如xx号), 不要输出代码, 也不需要输出额外的前缀、后缀. 如果没有人名则输出'WU-REN-MING'。@@@${line}@@@`,
        "format": {
            "type": "object",
            "properties": {
                "names": {
